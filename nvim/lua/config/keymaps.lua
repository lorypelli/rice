vim.keymap.set("n", "<leader>l", "<cmd>Lazy<CR>", { noremap = true, silent = true })
vim.keymap.set({ "n", "v" }, "<Tab>", function() vim.lsp.buf.hover() end, { noremap = true, silent = true })
vim.keymap.set({ "n", "v" }, "A", function() vim.lsp.buf.code_action() end, { noremap = true, silent = true })
vim.keymap.set("n", "<leader>q", "<cmd>:q<CR>", { noremap = true, silent = true })
vim.keymap.set("n", "<A-Up>", "<cmd>m -2<CR>", { noremap = true, silent = true })
vim.keymap.set("n", "<A-Down>", "<cmd>m +1<CR>", { noremap = true, silent = true })
vim.keymap.set("n", "<leader>w", "<cmd>split<CR>", { noremap = true, silent = true })
vim.keymap.set("n", "<leader>r", "<cmd>vsplit<CR>", { noremap = true, silent = true })
